 ...
 * @return {boolean} true 成功，false代表失败
 */
UtilsWrapper.prototype.collapseLogView = function () {
    if (utilsWrapper == null) {
        return null;
    }
    return utilsWrapper.collapseLogView();
};
 ...
 * @return {boolean} true 成功，false代表失败
 */
UtilsWrapper.prototype.expandLogView = function () {
    if (utilsWrapper == null) {
        return null;
    }
    return utilsWrapper.expandLogView();
};
 ...
 * @return {boolean} true 代表已经安装，false代表未安装
 */
 ...
 *
 * @param filePath 文件路径
 * @return {null|string} 例如com.tencent.mm
 */
 ...
 * @param jsonObject JSON对象
 * @param key        配置项目
 * @return {null|string} JSON中key对应的字符串数据
 */
...
UtilsWrapper.prototype.playMp3 = function (path, loop) {
    ...
    return utilsWrapper.playMp3(path, loop, false);
};
...
UtilsWrapper.prototype.playMp3WaitEnd = function (path, loop) {
    ...
    return utilsWrapper.playMp3(path, loop, true);
};
 ...
 * @return {null|Bitmap} Android的Bitmap对象，保存到文件请看image模块
 */
...
UtilsWrapper.prototype.unzipWithEncode = function (zipFile, passwd, destDir, fileNameEncode) {
    ...
    return utilsWrapper.unzip(zipFile, passwd, destDir, fileNameEncode);
};
/**
 * 将多个文件压缩成一个zip文件
 * @param zipFile 目标zip文件的路径
 ...
 * @param x 起始X位置
 * @param y 起始Y位置
 * @return {null|View} android的View对象
 */

...
/**
 * 执行全局的action动作
 * @param actionType 这个值请参考AccessibilityService下面的global开头的常量，例如：
 ...
 * @return {boolean} 布尔型| true代表成功
 */
function performGlobalAction(actionType) {
    if (isAccMode()) {
        return acEvent.performGlobalAction(actionType);
    } else if (isAgentMode()) {
        return agentEvent.performGlobalAction(actionType);
    }
    return false;
}
 ...
 * @param x         x坐标
 * @param y         y坐标
 * @param pointerId  手指参数 从1开始
 * @return {boolean} true 代表成功 false代表失败
 */
function touchDownEx(x, y, pointerId) {
    if (isAccMode()) {
        return acEvent.touchDown(x, y);
    } else if (isAgentMode()) {
        return agentEvent.touchDownEx(x, y, pointerId);
    }
    return false;
}
 ...
 * @return {boolean} true 代表成功 false代表失败
 */
function touchMoveEx(x, y, pointerId) {
    if (isAccMode()) {
        return acEvent.touchMove(x, y);
    } else if (isAgentMode()) {
        return agentEvent.touchMoveEx(x, y, pointerId);
    }
    return false;
}
/**
 ...
 * @return {boolean} true 代表成功 false代表失败
 */
function touchUpEx(x, y, pointerId) {
    if (isAccMode()) {
        return acEvent.touchUp(x, y);
    } else if (isAgentMode()) {
        return agentEvent.touchUpEx(x, y, pointerId);
    }
    return false;
}
...
/**
 * 重置touch状态
 * 防止因为touch执行的时候外部原因中断导致的不能再次执行touch
 * 无障碍模式不生效
 * 适合EC 11.16.0+
 * @return {boolean} true 代表成功 false代表失败
 */
function resetAllTouchState() {
    if (isAccMode()) {
        ...
        return agentEvent.resetAllTouchState();
    }
    return false;
}

 ...
 * 获取随机生成的机器设备ID，
 * 适配EC 11.8.0+
 * @return {string} 机器设备ID或者null
 */
ECloudWrapper.prototype.getDeviceId = function () {
    if (ecloudWrapper == null) {
        return;
    }
    return javaString2string(ecloudWrapper.getDeviceId());
};
...
/**
 * 设置云控参数, {"heartTime":5,"cloudUrl":"baidu.com"}
 * heartTime: 代表心跳时间
 * cloudUrl: 代表云控的链接地址
 */
ECloudWrapper.prototype.setCloudConfig = function (data) {
    if (ecloudWrapper == null) {
        return;
    }
    return ecImporter.setCloudConfig(JSON.stringify(data));
};
